...
- 메인 페이지: '사용자 맞춤 투자지원 AI' 소개, 스크롤 하여 둘러볼 수 있습니다.
- /analysis 페이지: AI에게 투자 포트폴리오를 제공받을 수 있습니다
- /portfolio 페이지: AI가 제공한 포트폴리오를 백테스팅하거나, 개별 기업에 대한 SpeedTraffic 분석을 할 수 있습니다.
- /speedtraffic 페이지: 개별 기업에 대한 분석 페이지, /portfolio에서 기업을 선택해 이동할 필요 없이 필요하다면 URL을 통해 직접 접근할 수 있습니다. (디버그 모드로 인식되나 기능 차이는 없음)
...
```mermaid
graph TD
    A[사용자 입력] --> B[1차 임베딩: 페르소나 분류<br/>bge-m3 임베딩]
    B --> C{투자가 아닌 다른 분류<br/>임계값이 큰가?}
    C -->|Yes| D[greeting/about_ai<br/>hcx-dash-002 응답]
    C -->|No| E[2차 임베딩: 산업 매칭<br/>63개 산업 벡터]
    E --> F{임계값 0.52<br/>이상?}
    F -->|Yes| G[단일 산업 선택]
    F -->|No| H[상위 2개 산업 추출]
...
### 📊 **데이터 정확성**
- **실제 거래 데이터**: yfinance 수정주가 기반 실시간 백테스팅, SpeedTraffic™ 분석
- **KOSPI 대부분의 산업 커버**: KRX 분류 상 속한 기업이 4개 이상인 모든 산업을 커버
- **실시간 뉴스 연동**: 네이버 뉴스 API 기반 최신 정보

### 🚀 **사용자 경험**
- **즉시 응답**: hcx-dash-002 모델의 빠른 1차 응답
- **직관적 신호등**: 복잡한 분석을 익숙한 신호등 개념으로 단순화

...
**동향 분석 올바른 예시:**
"최근 뉴스들을 종합하면, 정부의 ~~정책으로 인해 산업의 전반적인 분위기가 뜨거워지고 있습니다."
...
3. **동향 뉴스에서 산업을 아우르는 종합 인사이트를 추출하여 등의 종합적 인사이트를 표현하세요**
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                traditional_strategies: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      reason: {
                        type: 'string',
                        description: '1단계에서 추출된 해당 기업에 대해 개별 뉴스 2개 이상을 우선 인용하고, 필요시 동향 뉴스로 보완하여 구체적인 투자 근거 제시. 형식: "뉴스15에 따르면, 삼성전자는 AI 투자를 확대한다고 발표했습니다. 또한 뉴스23에서는 OO사와의 글로벌 파트너십 체결이 보도되었습니다. 이처럼 다각적 성장 전략으로 투자 매력도가 높습니다."'
                      }
                    },
                    required: ['ticker', 'name', 'reason']
                  },
                ...
                analysis_reasoning: {
                  type: 'string',
                  description: '동향 뉴스에서 종합적인 산업 인사이트를 추출하여 시장 분석 제시. 동향 뉴스 분석 시 특정 기업명 절대 언급 금지! "~~로 인한 시장의 급성장", "파트너십 확산", "기술 혁신 가속화" 등 산업 전반의 트렌드와 변화만 언급(단, 뉴스의 근거에 기반해야함). 이후 기업별 뉴스를 활용하여 구체적 분석. 최소 3개 이상의 서로 다른 뉴스를 인용하고, 각 뉴스 번호를 명시하여 풍부한 분석을 제공.'
                },
                strategy_comparison: {
                  type: 'string',
